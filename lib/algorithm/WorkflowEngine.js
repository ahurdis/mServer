...
function WorkflowEngine(options) {
    ...
    self.go = function (req, res) {
        ...
        if (_root) {
            let engineResult = runEngine().then(function (value) {
                ...
                let str = JSON.stringify([{ x: _root.instance + ' logged on server.' }]);
    ...
    let getVertexFunction = function (vertex) {
        ...
        if (vertex.type === 'UDFInControl' || vertex.type === 'UDFOutControl') {
            fRet = function (aro) { 
                return aro; 
            }
        }
                    ...
                    if (aro && Array.isArray(aro)) {
                        if (vertex.filterProperties && vertex.filterProperties.length > 0) {
                            for (var i = 0; i < aro.length; i++) {
                                arRet.push(_.pick(aro[i], vertex.filterProperties));
                            }
                        } else {
                            arRet = aro;
                        }
