...
function CodeGenerator(options) {
    ...
    self.generateCode = function () {
        ...
        try {
            ...
            importSet.add(`from pyspark.sql import SQLContext`)
                        .add(`from pyspark import SparkContext`);
                ...
                switch (vertex.type) {
                    ...
                    case 'FunctionControl':
                        switch (vertex.instance) {
                            case 'ToLowerCase':
                                code += `${vertex.varName} = ${upNames[0]}.toDF(*[c.lower() for c in ${upNames[0]}.columns])`;
                                break;
                            case 'ToUpperCase':
                                code += `${vertex.varName} = ${upNames[0]}.toDF(*[c.upper() for c in ${upNames[0]}.columns])`;
                                break;
                            case 'Join':
                                importSet.add(`from pyspark.sql.functions import monotonically_increasing_id`);
                                code += CommonTags.stripIndent`
